...
void GLCanvas::PanActiveCamera(const glm::vec2& pStartPoint, const glm::vec2& pEndPoint) {
    if (mFreeCamera != nullptr) {
        ...
        const glm::vec3 prevCamFrontVector =
            glm::normalize(mFreeCamera->GetLookAt() - prevCamPosition);
        ...
        newCamera->SetUp(glm::rotate(
            prevCamUpVector,
            glm::atan(initVector.y, initVector.x) - glm::atan(finalVector.y, finalVector.x),
            prevCamFrontVector));
...
void GLCanvas::paintGL() {
    if (mScene != nullptr) {
        ...
        for (;;) {
            ...
            mShaderDualPeel->BindTexture(GL_TEXTURE_RECTANGLE, "DepthBlenderTex",
                                         mDualDepthTexId[prevId], 1);
            mShaderDualPeel->BindTexture(GL_TEXTURE_RECTANGLE, "FrontBlenderTex",
            ...
            mShaderDualPeelPerVertexColor->BindTexture(GL_TEXTURE_RECTANGLE, "DepthBlenderTex",
                                                       mDualDepthTexId[prevId], 1);
            mShaderDualPeelPerVertexColor->BindTexture(GL_TEXTURE_RECTANGLE, "FrontBlenderTex",
                                                       mDualFrontBlenderTexId[prevId], 2);
            ...
            mShaderDualBlend->BindTexture(GL_TEXTURE_RECTANGLE, "TempTex",
                                          mDualBackTempTexId[currId], 1);
        ...
        mShaderDualFinal->BindTexture(GL_TEXTURE_RECTANGLE, "FrontBlenderTex",
                                      mDualFrontBlenderTexId[currId], 2);
        mShaderDualFinal->BindTexture(GL_TEXTURE_RECTANGLE, "BackBlenderTex", mDualBackBlenderTexId,
                                      3);
        mMeshFullScreenQuad->Draw();
        glUseProgram(0);
...
void GLCanvas::InitDualPeelingRenderTargets() {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        glTexImage2D(GL_TEXTURE_RECTANGLE, 0, GL_RG32F, mWinWidth, mWinHeight, 0, GL_RG, GL_FLOAT,
                     0);
        ...
        glTexParameteri(GL_TEXTURE_RECTANGLE, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_RECTANGLE, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        glTexImage2D(GL_TEXTURE_RECTANGLE, 0, GL_RGBA, mWinWidth, mWinHeight, 0, GL_RGBA, GL_FLOAT,
                     0);
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_RECTANGLE,
                           mDualDepthTexId[j], 0);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_RECTANGLE,
                           mDualFrontBlenderTexId[j], 0);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_TEXTURE_RECTANGLE,
                           mDualBackTempTexId[j], 0);

    j = 1;
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_RECTANGLE,
                           mDualDepthTexId[j], 0);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT4, GL_TEXTURE_RECTANGLE,
                           mDualFrontBlenderTexId[j], 0);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT5, GL_TEXTURE_RECTANGLE,
                           mDualBackTempTexId[j], 0);

    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT6, GL_TEXTURE_RECTANGLE,
                           mDualBackBlenderTexId, 0);
