...
void SerializedSceneGeometry::ComputeBoundingSphere() {
    ...
    if (!mb.is_valid(1e-6)) {
        ...
        Debug::Warning("Bounding sphere in SerializedSceneGeometry may be invalid. Accuracy: " +
                       std::to_string(error));
    }
...
void SerializedSceneGeometry::ComputeNeighbourhood() {
    ...
    Debug::Log("SerializedSceneGeometry::ComputeNeighbourhood: Epsilon value: " +
               std::to_string(epsilon));

    std::vector<std::pair<size_t, glm::vec3>> points(mNumberOfFaces * 3);
    // All vertices of the scene are added to a list of points with information about what mesh they
    // belong, the vertex index inside the mesh and the vertex position
    ...
    for (size_t j = 0; j < mNumberOfFaces * 3; j++) {
        std::vector<size_t> neighboursX =
            Tools::FindNearestThanEpsilonByDimension(pointsX.at(j), pointsOrderedByX, epsilon, 0);
        std::vector<size_t> neighboursY =
            Tools::FindNearestThanEpsilonByDimension(pointsY.at(j), pointsOrderedByY, epsilon, 1);
        std::vector<size_t> neighboursZ =
            Tools::FindNearestThanEpsilonByDimension(pointsZ.at(j), pointsOrderedByZ, epsilon, 2);
        std::vector<size_t> neighbours =
            Tools::MergeNeighbours(neighboursX, neighboursY, neighboursZ);
    ...
    for (int j = 0; j < mVertexNeighbors.size(); j++) {
        ...
        std::vector<size_t>::iterator it =
            std::unique(mVertexNeighbors[j].begin(), mVertexNeighbors[j].end());
    ...
    for (size_t j = 0; j < mNumberOfFaces; j++) {
        ...
        for (int k = 0; k < 3; k++) {
            ...
            polygonNeighbours.insert(polygonNeighbours.end(), vertexNeighbors.cbegin(),
                                     vertexNeighbors.cend());
        }

        std::sort(polygonNeighbours.begin(), polygonNeighbours.end());
        std::vector<size_t>::iterator it =
            std::unique(polygonNeighbours.begin(), polygonNeighbours.end());
    ...
    Debug::Log("Scene::ComputeNeighbourhood - Total time elapsed: " +
...
void SerializedSceneGeometry::ComputeFaceAreas() {
    for (size_t i = 0; i < mNumberOfFaces; i++) {
        mFaceAreas[i] = 0.5f * glm::length(glm::cross(
                                   mVertexs.at(mFaces.at(i)[1]) - mVertexs.at(mFaces.at(i)[0]),
                                   mVertexs.at(mFaces.at(i)[2]) - mVertexs.at(mFaces.at(i)[0])));
    }
}
...
std::vector<glm::vec3> SerializedSceneGeometry::GetVertices() const { return mVertexs; }
...
std::vector<float> SerializedSceneGeometry::GetFacesAreas() const { return mFaceAreas; }
    ...
    for (int i = 0; i < pSceneNode->GetNumMeshes(); i++) {
        ...
        if (geom->GetTopology() == GeometryTopology::Triangles) {
            ...
            for (int j = 0; j < indexs.size(); j += 3) {
                glm::ivec3 face(indexs.at(j) + facesOffset, indexs.at(j + 1) + facesOffset,
                                indexs.at(j + 2) + facesOffset);
                mFaces[mNumberOfFaces] = face;
                mNumberOfFaces++;
            }
        } else {
            Debug::Error(
                "The serialization only works when the scene is only compunded by triangles");
        }
    }
