...
Scene::Scene(const std::string& pName, std::shared_ptr<SceneNode> pSceneRoot,
...
void Scene::ShowInformation() const {
    ...
    Debug::Log("  Center: (" + std::to_string(sceneCenter.x) + ", " +
               std::to_string(sceneCenter.y) + ", " + std::to_string(sceneCenter.z) + ")");
