...
void GPUGeometry::SetVerticesData(const std::vector<float>& pVertices, unsigned int pStride) {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 static_cast<GLsizeiptr>(pVertices.size() *
                                         sizeof(std::decay_t<decltype(pVertices)>::value_type)),
...
void GPUGeometry::SetNormalsData(const std::vector<float>& pNormals) {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 static_cast<GLsizeiptr>(pNormals.size() *
                                         sizeof(std::decay_t<decltype(pNormals)>::value_type)),
                 pNormals.data(), GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}
...
void GPUGeometry::SetColorData(const std::vector<float>& pColors, unsigned int pStride) {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 static_cast<GLsizeiptr>(pColors.size() *
                                         sizeof(std::decay_t<decltype(pColors)>::value_type)),
                 pColors.data(), GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}
...
void GPUGeometry::SetTangentData(const std::vector<float>& pTangents) {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 static_cast<GLsizeiptr>(pTangents.size() *
                                         sizeof(std::decay_t<decltype(pTangents)>::value_type)),
                 pTangents.data(), GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}
...
void GPUGeometry::SetBitangentData(const std::vector<float>& pBitangents) {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 static_cast<GLsizeiptr>(pBitangents.size() *
                                         sizeof(std::decay_t<decltype(pBitangents)>::value_type)),
                 pBitangents.data(), GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}
...
void GPUGeometry::SetTextCoordsData(const std::vector<float>& pTextCoords) {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 static_cast<GLsizeiptr>(pTextCoords.size() *
                                         sizeof(std::decay_t<decltype(pTextCoords)>::value_type)),
                 pTextCoords.data(), GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}

void GPUGeometry::SetIndexsData(const std::vector<unsigned int>& pIndexs,
                                GeometryTopology pTopology) {
    if (mIndexsId == 0) {
    ...
    glBufferData(
        ...
        static_cast<GLsizeiptr>(mIndexsSize * sizeof(std::decay_t<decltype(pIndexs)>::value_type)),
        pIndexs.data(), GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
}

void GPUGeometry::FastDraw(unsigned int pSize) const {
    glDrawElements(static_cast<GLenum>(mMeshTopology), static_cast<GLsizei>(pSize), GL_UNSIGNED_INT,
                   nullptr);
}
...
void GPUGeometry::ConfigureVAO() {
    ...
    if (mVerticesId != 0) {
        glBindBuffer(GL_ARRAY_BUFFER, mVerticesId);
        glVertexAttribPointer(POSITION, static_cast<int>(mVerticesStride), GL_FLOAT, GL_FALSE, 0,
                              nullptr);
    ...
    if (mColorsId != 0) {
        glBindBuffer(GL_ARRAY_BUFFER, mColorsId);
        glVertexAttribPointer(COLOR, static_cast<int>(mColorsStride), GL_FLOAT, GL_FALSE, 0,
                              nullptr);
