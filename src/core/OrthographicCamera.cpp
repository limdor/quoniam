...
OrthographicCamera::OrthographicCamera(float pNearPlane, float pFarPlane, glm::vec3 pLookAt,
                                       glm::vec3 pUp, glm::vec3 pPosition, float pTop,
                                       float pBottom, float pLeft, float pRight)
    : Camera(pNearPlane, pFarPlane, pLookAt, pUp, pPosition, (pRight - pLeft) / (pTop - pBottom)),
      mTop(pTop),
      mBottom(pBottom),
      mLeft(pLeft),
      mRight(pRight)
...
void OrthographicCamera::UpdatePositions()
{
    ...
    mPositionOfVertices[4] =
        glm::vec4(mPosition + upVector * mTop + leftVector * mLeft + frontVector, 1.0f);
    mPositionOfVertices[5] =
        glm::vec4(mPosition + upVector * mTop - leftVector * mRight + frontVector, 1.0f);
    mPositionOfVertices[6] =
        glm::vec4(mPosition - upVector * mBottom - leftVector * mRight + frontVector, 1.0f);
    mPositionOfVertices[7] =
        glm::vec4(mPosition - upVector * mBottom + leftVector * mLeft + frontVector, 1.0f);
