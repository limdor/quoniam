...
BoundingSphere::BoundingSphere() : Gizmo(), mCenter(0.0f), mRadius(0.0f) { CreateMesh(); }

glm::vec3 BoundingSphere::GetCenter() const { return mCenter; }

float BoundingSphere::GetRadius() const { return mRadius; }
...
std::shared_ptr<BoundingSphere> BoundingSphere::Merge(std::shared_ptr<BoundingSphere const> pBS0,
                                                      std::shared_ptr<BoundingSphere const> pBS1) {
    auto result = std::make_shared<BoundingSphere>();

    if (pBS0 != nullptr && pBS1 != nullptr) {
        // Bibliography: 3D Game Engine Design: A Practical Approach to Real-Time Computer Graphics
        // (p.149)
...
void BoundingSphere::UpdatePositions() {
    ...
    for (float t = 0.0f; t <= pi2; t += step) {
        mPositionOfVertices[index] = glm::vec4(mCenter.x + glm::cos(t) * mRadius, mCenter.y,
                                               mCenter.z + glm::sin(t) * mRadius, 1.0f);
        index++;
    }

    for (float t = 0.0f; t <= pi2; t += step) {
        mPositionOfVertices[index] = glm::vec4(mCenter.x, mCenter.y + glm::cos(t) * mRadius,
                                               mCenter.z + glm::sin(t) * mRadius, 1.0f);
        index++;
    }

    for (float t = 0.0f; t <= pi2; t += step) {
        mPositionOfVertices[index] = glm::vec4(mCenter.x + glm::cos(t) * mRadius,
                                               mCenter.y + glm::sin(t) * mRadius, mCenter.z, 1.0f);
        index++;
    }
