...
GLSLProgram::GLSLProgram(const std::string& pName) : mName(pName), mId(glCreateProgram()) {
    CHECK_GL_ERROR();
}

GLSLProgram::~GLSLProgram() { glDeleteProgram(mId); }

const std::string& GLSLProgram::GetName() const { return mName; }
...
void GLSLProgram::UseProgram() const { glUseProgram(mId); }
...
void GLSLProgram::BindTexture(GLenum pTarget, const std::string& pTextureName, GLuint pTextureId,
                              int pTextureUnit) {
