...
void TrackballCamera::MoveCamera(const glm::vec2& pMouseInitialPosition,
    ...
    if (moviment != glm::vec2{0, 0}) {
        ...
        const float rotationAmount =
            glm::sqrt(moviment.x * (float)moviment.x + moviment.y * moviment.y / 5.0f);
        const glm::vec3 leftVector =
            glm::normalize(glm::cross(prevCamUpVector, prevCamFrontVector));
        glm::mat4 transform = glm::translate(glm::mat4(1.0f), pCenter);
        const glm::vec3 rotationVector =
            glm::normalize(prevCamUpVector * -(float)moviment.x + leftVector * (float)moviment.y);
