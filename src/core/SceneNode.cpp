...
std::string SceneNode::GetName() const { return mName; }
...
glm::mat4 SceneNode::GetLocalTransform() const { return mLocalTransform; }

glm::mat4 SceneNode::GetGlobalTransform() const { return mGlobalTransform; }

size_t SceneNode::GetNumberOfVertices() const { return mNumberOfVertices; }

size_t SceneNode::GetNumberOfPolygons() const { return mNumberOfPolygons; }
...
void SceneNode::AddCamera(Camera* pCamera) { mCameras.push_back(pCamera); }
...
std::shared_ptr<SceneNode> SceneNode::GetParent() const { return mParent; }
...
size_t SceneNode::GetNumMeshes() const { return mMeshes.size(); }

std::shared_ptr<Mesh const> SceneNode::GetMesh(size_t pPosition) const {
    return mMeshes.at(pPosition);
}

size_t SceneNode::GetNumCameras() const { return mCameras.size(); }

const Camera* SceneNode::GetCamera(size_t pPosition) const { return mCameras.at(pPosition); }

size_t SceneNode::GetNumChilds() const { return mChilds.size(); }
