...
void PerspectiveCamera::CreateMesh()
{
    ...
    /// Set the indexs
    std::vector<unsigned int> indexs(16);
    // clang-format off
...
void PerspectiveCamera::UpdatePositions()
{
    ...
    mPositionOfVertices[0] =
        glm::vec4(mPosition + frontVector + upVector + leftVector * mAspectRatio, 1.0f);
    mPositionOfVertices[1] =
        glm::vec4(mPosition + frontVector + upVector - leftVector * mAspectRatio, 1.0f);
    mPositionOfVertices[2] =
        glm::vec4(mPosition + frontVector - upVector - leftVector * mAspectRatio, 1.0f);
    mPositionOfVertices[3] =
        glm::vec4(mPosition + frontVector - upVector + leftVector * mAspectRatio, 1.0f);
