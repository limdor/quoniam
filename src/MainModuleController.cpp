...
MainModuleController::MainModuleController(QWidget* pParent)
    : ModuleController(pParent),
      mUi(new Ui::MainModule),
      mDutagaciDialog(nullptr),
      mUpdateView(false),
      mScene(nullptr),
    ...
    const LightSettings light_1_settings{
        mUi->light1GroupBox->isChecked(), glm::vec3(1.0f, 1.0f, 0.0f),
        glm::vec3(light1InitialColor.redF(), light1InitialColor.greenF(),
                  light1InitialColor.blueF())};

    const LightSettings light_2_settings{
        mUi->light2GroupBox->isChecked(), glm::vec3(0.0f, -1.0f, 0.0f),
        glm::vec3(light2InitialColor.redF(), light2InitialColor.greenF(),
                  light2InitialColor.blueF())};
    ...
    mOpenGLCanvas->SetAmbientLightIntensity(
        static_cast<float>(mUi->ambientLightAmountSpinBox->value()));
    ...
    // To use depth-based visual stability uncomment this line, it can take a lot of time to be
    // computed mViewpointMeasures.push_back( new DepthBasedVisualStability("Depth-based visual
    // stability") );
    ...
    for (size_t i = 0; i < mViewpointMeasures.size(); i++) {
        mUi->measureInViewpointSphereList->addItem(
            QString::fromStdString(mViewpointMeasures.at(i)->GetName()));
    }
    ...
    for (size_t i = 0; i < mViewpointMeasures.size(); i++) {
        ...
        ViewpointMeasureSlider* slider =
            new ViewpointMeasureSlider(static_cast<int>(i), Qt::Horizontal);
    ...
    for (int i = 0; i < 4; i++) {
        mUi->numberOfViewpoints->addItem(
            QString("%1").arg(glm::pow(2.0f, 2.0f * mUi->numberOfViewpoints->count()) * 10 + 2));
    }
...
void MainModuleController::CreateModuleMenus() {
    ...
    connect(actionBoundingBoxes, SIGNAL(triggered(bool)), mOpenGLCanvas,
            SLOT(WillDrawBoundingBox(bool)));
    ...
    connect(actionBoundingSpheres, SIGNAL(triggered(bool)), mOpenGLCanvas,
            SLOT(WillDrawBoundingSphere(bool)));
    ...
    connect(mActionViewpointsSphere, SIGNAL(triggered(bool)), this,
            SLOT(WillDrawViewpointsSphere(bool)));
...
void MainModuleController::ActiveModule() { Debug::SetConsole(mUi->console); }
...
void MainModuleController::keyPressEvent(QKeyEvent* pEvent) {
    if (pEvent->key() == Qt::Key_N && mSphereOfViewpoints != nullptr) {
        ...
        Debug::Log("Viewpoint " + mSphereOfViewpoints->GetViewpoint(viewpoint)->mName +
                   " selected");

        mUpdateView = false;
        for (size_t i = 0; i < mViewpointMeasuresSliders.size(); i++) {
            mViewpointMeasuresSliders.at(i)->setValue(
                static_cast<int>(mViewpointMeasures.at(i)->GetPosition(viewpoint)));
        }
        mUpdateView = true;
...
void MainModuleController::LoadScene(const std::filesystem::path& pFileName) {
    ...
    Debug::Log("MainWindow::LoadScene - Total time elapsed: " +
               std::to_string(timeElapsed.count()) + " ms");
...
void MainModuleController::LoadViewpoints(int pWidthResolution, bool pFaceCulling) {
    ...
    mSceneInformationBuilder->CreateHistogram(mScene, mSphereOfViewpoints, pWidthResolution,
                                              pFaceCulling, true);
    ...
    // Compute the viewpoint measures only if they have not being caluclated before through
    // dependencies
    for (int i = 0; i < mViewpointMeasures.size(); i++) {
        if (!mViewpointMeasures.at(i)->Computed()) {
            progress.setLabelText(
                QString("Computing %1...")
                    .arg(QString::fromStdString(mViewpointMeasures.at(i)->GetName())));
    ...
    auto const end = std::chrono::steady_clock::now();
    std::chrono::duration<double, std::milli> const timeElapsed = end - start;
    Debug::Log("MainWindow::Measures computed - Time elapsed: " +
               std::to_string(timeElapsed.count()) + " ms");
    QApplication::restoreOverrideCursor();

    on_measureInViewpointSphereList_currentIndexChanged(
        mUi->measureInViewpointSphereList->currentIndex());
...
void MainModuleController::LoadViewpointsFromSphere(float pRadius, float pAngle, float pAspectRatio,
                                                    unsigned char pSubdivision,
                                                    int pWidthResolution, bool pFaceCulling) {
    ...
    if (file.open(QFile::WriteOnly)) {
        ...
        for (int i = 0; i < mSphereOfViewpoints->GetNumberOfViewpoints(); i++) {
            ...
            stream.writeAttribute(
                "name", QString::fromStdString(mSphereOfViewpoints->GetViewpoint(i)->mName));
            for (int j = 0; j < mViewpointMeasures.size(); j++) {
                ...
                name = name.replace("-", "_");
                name = name.replace("|", "_");
                stream.writeTextElement(QString("%1").arg(name),
                                        QString("%1").arg(mViewpointMeasures.at(j)->GetValue(i)));
            }
...
size_t MainModuleController::NextViewpoint() {
    mCurrentViewpoint++;
    if (mCurrentViewpoint >= mSphereOfViewpoints->GetNumberOfViewpoints()) mCurrentViewpoint = 0;
...
void MainModuleController::SetViewpoint(size_t pViewpoint) {
    ...
    for (size_t i = 0; i < mViewpointMeasuresSliders.size(); i++) {
        mViewpointMeasuresSliders.at(i)->setValue(
            static_cast<int>(mViewpointMeasures.at(i)->GetPosition(pViewpoint)));
    }
    mUpdateView = true;
}
...
void MainModuleController::ShowViewpointInformation(size_t pViewpoint) {
    ...
    Debug::Log("Normalized position: " + std::to_string(position.x) + ", " +
               std::to_string(position.y) + ", " + std::to_string(position.z));
    Debug::Log("Viewpoint quality measures:");
    for (size_t i = 0; i < mViewpointMeasures.size(); i++) {
        Debug::Log("  " + mViewpointMeasures.at(i)->GetName() + ": " +
                   std::to_string(mViewpointMeasures.at(i)->GetValue(pViewpoint)));
    }
}
...
void MainModuleController::LoadDutagaciViewpoints() {
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (!textReader.atEnd()) {
            ...
            if (list.size() == 3) {
                mDutagaciViewpoints.push_back(
                    glm::vec3(list.at(0).toFloat(), list.at(1).toFloat(), list.at(2).toFloat()));
            } else {
...
void MainModuleController::OpenModel() {
    const QString fileName = QFileDialog::getOpenFileName(
        ...
        tr("Supported models (*.3ds *.dae *.off *.ply *.obj);;3ds Max 3DS (*.3ds);;Collada "
           "(*.dae);;Object File Format (*.off);;Stanford Polygon Library (*.ply);;Wavefront "
           "Object (*.obj);;All files (*.*)"));
    if (!fileName.isEmpty()) {
...
void MainModuleController::ExportInformation() {
    const QString fileName = QFileDialog::getSaveFileName(this, tr("Choose a file to export"), "./",
    ...
    if (dialogReturn == QDialog::Accepted) {
        ...
        for (size_t i = 0; i < numberOfFiles; i++) {
            ...
            for (size_t j = 0; j < mViewpointMeasures.size(); j++) {
                glm::vec3 position =
                    mSphereOfViewpoints->GetViewpoint(mViewpointMeasures.at(j)->GetBestViewpoint())
                        ->GetPosition();
                position = (position - center) / (radius * 3.0f);
        ...
        for (size_t i = 0; i < mViewpointMeasures.size(); i++) {
            QString fileName = QString::fromStdString(mViewpointMeasures.at(i)->GetName())
                                   .replace("|", "_")
                                   .replace("/", "_") +
                               ".txt";
            QFile file(fileName);
            if (file.open(QFile::WriteOnly)) {
                ...
                for (int j = 0; j < numberOfFiles; j++) {
                    out << bestViewpoints.at(i).at(j).x << " " << bestViewpoints.at(i).at(j).y
                        << " " << bestViewpoints.at(i).at(j).z << "\n";
                }
                file.close();
...
void MainModuleController::on_measureInViewpointSphereList_currentIndexChanged(int pValue) {
    if (mSphereOfViewpoints != nullptr) {
        ...
        if (pValue < mViewpointMeasures.size()) {
            colors = Tools::ConvertFloatsToColors(mViewpointMeasures.at(pValue)->GetValues(),
                                                  !mViewpointMeasures.at(pValue)->IsMaximumBest());
        }
...
void MainModuleController::on_loadViewpointsSphereButton_clicked() {
    ...
    LoadViewpointsFromSphere(mUi->cameraDistanceSpinBox->value(), mUi->cameraAngleSpinBox->value(),
                             mUi->cameraAspectRatioSpinBox->value(), subdivision,
                             mUi->widthResolutionSpinBox->value(),
                             mUi->faceCullingCheckBox->isChecked());
}
...
void MainModuleController::on_light1ColorButton_clicked() {
    QColor color =
        QColorDialog::getColor(mUi->light1ColorLabel->palette().color(QPalette::Window), this);
    if (color.isValid()) {
...
void MainModuleController::on_light2ColorButton_clicked() {
    QColor color =
        QColorDialog::getColor(mUi->light2ColorLabel->palette().color(QPalette::Window), this);
    if (color.isValid()) {
...
void MainModuleController::on_bestAndWorstViewsButton_clicked() {
    ...
    for (size_t measureIndex = 0; measureIndex < numberOfMeasures; measureIndex++) {
        ...
        if (measure->IsMaximumBest()) {
            mOpenGLCanvas->SaveScreenshot(
                QString("%1_Worst_%2.png").arg(sceneName).arg(measureName));
        } else {
            mOpenGLCanvas->SaveScreenshot(
                QString("%1_Best_%2.png").arg(sceneName).arg(measureName));
        }
        ...
        if (measure->IsMaximumBest()) {
            mOpenGLCanvas->SaveScreenshot(
                QString("%1_Worst_%2_Sphere.png").arg(sceneName).arg(measureName));
        } else {
            mOpenGLCanvas->SaveScreenshot(
                QString("%1_Best_%2_Sphere.png").arg(sceneName).arg(measureName));
        }
        mOpenGLCanvas->ResetActiveCamera();
