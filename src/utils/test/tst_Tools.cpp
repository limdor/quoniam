...
{
    const QVector<float> input{2.0f, 4.0f, 1.0f, 3.0f};

    const QVector<int> result = Tools::GetOrderedIndexes(input);
    const QVector<int> expected{2, 0, 3, 1};
    REQUIRE(result == expected);
}

TEST_CASE("GetPositions", "Tools")
{
    const QVector<int> input{2, 4, 1, 3};

    const QVector<int> result = Tools::GetPositions(input);
    const QVector<int> expected{2, 0, 3, 1};
    REQUIRE(result == expected);
}
