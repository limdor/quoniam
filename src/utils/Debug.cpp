...
bool Debug::CheckGLError(std::string const& pFile, int pLine)
{
    ...
    for( auto const& error : errors )
    {
        ...
        Debug::Error("GL Error #" + std::to_string(error_code) + "(" + optional_message +
                     ") in file " + pFile + " at line: " + std::to_string(pLine));
    }
    return retCode;
}
    ...
    switch( pType )
    {
        ...
        case QtDebugMsg:
            mConsole->appendHtml(QString("<FONT color=black>%1</FONT>")
                                     .arg(QString(pMessage).replace(" ", "&nbsp;")));
            fprintf(stdout, "Log: %s\n", pMessage.toLocal8Bit().constData());
            fflush(stdout);
            break;
        case QtWarningMsg:
            mConsole->appendHtml(QString("<FONT color=yellow>%1</FONT>")
                                     .arg(QString(pMessage).replace(" ", "&nbsp;")));
        ...
        case QtFatalMsg:
            mConsole->appendHtml(
                QString("<FONT color=red>%1</FONT>").arg(QString(pMessage).replace(" ", "&nbsp;")));
