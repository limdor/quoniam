...
template <typename T>
std::vector<T> Tools::GetOrderedIndexesByDimension(std::vector<std::pair<T, glm::vec3>>& pValues,
                                                   int pDimension) {
    switch (pDimension) {
        case 0:
    ...
    std::vector<T> result;
    std::transform(pValues.cbegin(), pValues.cend(), std::back_inserter(result),
                   [](std::pair<T, glm::vec3> pair) -> T { return pair.first; });

    return result;
}

template std::vector<int> Tools::GetOrderedIndexesByDimension(
    std::vector<std::pair<int, glm::vec3>>& pValues, int pDimension);
template std::vector<size_t> Tools::GetOrderedIndexesByDimension(
    std::vector<std::pair<size_t, glm::vec3>>& pValues, int pDimension);
...
std::vector<std::size_t> Tools::GetOrderedIndexes(const std::vector<T>& pValues) {
    ...
    std::ranges::sort(toSort,
                      [](std::pair<size_t, T> const& i, std::pair<size_t, T> const& j) -> bool {
...
std::vector<std::pair<std::size_t, T>> Tools::addIndex(const std::vector<T>& values) {
    auto makePair = [](T const& value, std::size_t index) -> std::pair<std::size_t, T> {
        return std::make_pair(index, value);
    };
    std::vector<std::pair<std::size_t, T>> valuesWithIndex;
    valuesWithIndex.reserve(values.size());
    std::ranges::transform(values, std::views::iota(0), std::back_inserter(valuesWithIndex),
                           makePair);
...
template std::vector<std::pair<std::size_t, float>> Tools::addIndex(
    const std::vector<float>& values);
template std::vector<std::pair<std::size_t, std::size_t>> Tools::addIndex(
    const std::vector<std::size_t>& values);

std::vector<glm::vec4> Tools::ConvertFloatsToColors(const std::vector<float>& pValues,
                                                    bool pInverted) {
    return ConvertNormalizedFloatsToColors(ScaleValues(pValues, 0.0f, 1.0f), pInverted);
}

std::vector<float> Tools::ScaleValues(const std::vector<float>& pValues, float pLowerBound,
                                      float pUpperBound, float pPercentOfClipping) {
    float min, max;
...
template <typename T>
std::vector<T> Tools::FindNearestThanEpsilonByDimension(
    ...
    while (nextUp || nextDown) {
        if (nextUp) {
            if ((pPosition + i) < pVector.size()) {
                if (glm::abs(pVector.at(pPosition + i).second[pDimension] -
                             pVector.at(pPosition).second[pDimension]) < pEpsilon) {
...
template std::vector<int> Tools::FindNearestThanEpsilonByDimension(
    size_t pPosition, const std::vector<std::pair<int, glm::vec3>>& pVector, float pEpsilon,
    int pDimension);
template std::vector<size_t> Tools::FindNearestThanEpsilonByDimension(
    size_t pPosition, const std::vector<std::pair<size_t, glm::vec3>>& pVector, float pEpsilon,
    int pDimension);

template <typename T>
std::vector<T> Tools::MergeNeighbours(const std::vector<T>& pVector1,
...
template std::vector<int> Tools::MergeNeighbours(const std::vector<int>& pVector1,
                                                 const std::vector<int>& pVector2,
                                                 const std::vector<int>& pVector3);
template std::vector<size_t> Tools::MergeNeighbours(const std::vector<size_t>& pVector1,
                                                    const std::vector<size_t>& pVector2,
                                                    const std::vector<size_t>& pVector3);

std::vector<glm::vec4> Tools::ConvertNormalizedFloatsToColors(const std::vector<float>& pValues,
                                                              bool pInverted) {
    std::vector<glm::vec4> results;
    std::transform(pValues.cbegin(), pValues.cend(), std::back_inserter(results),
                   [pInverted](float value) -> glm::vec4 {
                       return ConvertNormalizedFloatToColor(value, pInverted);
                   });
...
std::filesystem::path Tools::GetProgramPath() {
    ...
    std::string const program_folder{
        "Quoniam"};  // TODO: Change to constexpr once supported by the compiler
