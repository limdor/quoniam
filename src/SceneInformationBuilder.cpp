    ...
    int pWidthResolution, bool pFaceCulling, bool pIgnoreNormals)
{
    ...
    mProjectedAreasMatrix =
        std::make_shared<ProjectedAreasMatrix>(numberOfViewpoints, numberOfPolygons);
    ...
    for( size_t i = 0; i < numberOfViewpoints; i++ )
    {
        ...
        for( int k = 0; k < gpuScene.GetNumberOfSceneNodes(); k++ )
        {
            ...
            mShaderColorPerFace->SetUniform("offset",
                                            static_cast<int>(sceneNode->GetPolygonalOffset()));
    ...
    Debug::Log("GLCanvas::ComputeViewpointsProbabilities " + std::to_string(windowWidth) + "x" +
               std::to_string(windowHeight) +
               " - Time elapsed: " + std::to_string(timeElapsed.count()) + " ms");
...
void SceneInformationBuilder::RestoreOpenGLStats()
{
    ...
    glClearColor(mPreviousClearColor.r, mPreviousClearColor.g, mPreviousClearColor.b,
                 mPreviousClearColor.a);
    glViewport(mPreviousViewport[0], mPreviousViewport[1], mPreviousViewport[2],
               mPreviousViewport[3]);
}
